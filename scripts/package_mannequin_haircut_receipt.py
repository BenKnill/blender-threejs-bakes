#!/usr/bin/env python3
"""Publish a compact tracked summary for the mannequin haircut bake."""
...
def main() -> int:
    ...
    receipt = {
        "schema": "mannequin-haircut-bake-summary/1",
        "platform_scope": "observed on the primary M5 Mac; not a portable benchmark",
        "fixture": {
            "frames": render["frames"],
            "resolution": [960, 540],
            "physical_guides": render["physical_guides"],
            "render_fibers": render["fibers"],
            "points_per_fiber": render["points_per_fiber"],
            "severed_fibers": render["severed_fibers"],
            "cut_time_s": render["cut_time_s"],
            "cut_height_m": render["cut_height_m"],
        },
        "stages": {"blender_frames": blender, "video_packaging": video},
        "headroom_gates": {
            "blender_under_180_seconds": blender["wall_seconds"] < 180,
            "blender_peak_under_2_gib": blender["peak_rss_bytes"] < 2 * 1024**3,
            "all_97_frames_present": blender["artifacts"][0].get("file_count") == 97,
        },
        "scale_decision": (
            f"{render['fibers']} fine render fibers were selected after a 480-fiber "
            "probe remained comfortably laptop-safe"
        ),
        "claim_boundary": {
            "continuum_hair_mechanics": render["continuum_hair_mechanics"],
            "strand_self_contact": render["strand_self_contact"],
            "cutting_tool_contact_mechanics": render["cutting_tool_contact_mechanics"],
        },
